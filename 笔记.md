...
> 不能在无状态组件中使用( 函数组件 )，因为无状态组件没有实例。官方文档中不建议我们过度使用 refs 对逻辑进行处理，需要优先考虑 state
...
> 事件对象不是原生 JavaScript 事件，是合成事件对象( nodejs 的 event 对象 )
...
```js
// mobx 中一个文件就是一个分片数据
// observable 用与定义 state
// action 不用多说...
// computed 当 observable 中的数据发生变化时就会触发，类似于 Vue 中的 compoted 属性
